...
local function is_clipboard_img(content)
  ...
  if this_os == util.OS.Linux then
    return vim.tbl_contains(content, "image/png")
  elseif this_os == util.OS.Darwin then
...
---@param get_text function (Path,) -> string
M.paste_img = function(fname, default_dir, get_text)
  local cmd_check, cmd_paste = get_clip_command()
  local content = get_clip_content(cmd_check)
  if not is_clipboard_img(content) then
    log.error "There is no image data in the clipboard"
  else
    ...
    util.insert_text(get_text(path))
  end
end
